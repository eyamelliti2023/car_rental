...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if($valid == 1){
        ...
        $carC->updatecar($car, $id); // Assuming this adds the car to the database
...
<div class="container mt-5">
    ...
    <form method="post" action="" enctype="multipart/form-data" class="p-4 border rounded shadow-sm">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="price" name="price" value="<?= htmlspecialchars($car['price_per_day']) ?>" required>
        </div>

        <!-- Colour -->
        <div class="mb-3">
            <label for="colour" class="form-label">Colour (Actual Colour)</label>
            <input type="text" class="form-control" id="colour" name="colour" value="<?= htmlspecialchars($car['color']) ?>" required>
        </div>
        ...
        <div class="mb-3">
            ...
            <select class="form-select" id="transmission" name="transmission" required>
                <option value="1" <?= $car['transmission'] == 1 ? 'selected' : '' ?>>Automatic</option>
                <option value="2" <?= $car['transmission'] == 2 ? 'selected' : '' ?>>Manual</option>
            </select>
        </div>
        ...
        <div class="mb-3">
            ...
            <?php if (!empty($car['image'])): ?>
                <p>Current Image: <img src="<?= $car['image'] ?>" alt="Car Image" width="100"></p>
            <?php endif; ?>
        </div>
